...
type
  TMuCRC32 = class
    public
      class function GetCRC32(const aFileName: string; var filecrc: longword): boolean; overload;
      class function GetCRC32(const aStream: tStream; var filecrc: longword): boolean; overload;
      class function GetCRC32(const aBytes: TBytes; var filecrc: longword): boolean; overload;

      class function GetCRC32Pos(const aFileName: string; startoffset, endoffset: int64; var filecrc: longword)
        : boolean; overload;
      class function GetCRC32Pos(const aStream: tStream; startoffset, endoffset: int64; var filecrc: longword)
        : boolean; overload;
...
const
  crctable: array [0 .. 255] Of longword = ($00000000, $77073096, $EE0E612C, $990951BA, $076DC419, $706AF48F, $E963A535,
...
class function TMuCRC32.GetCRC32(const aStream: tStream; var filecrc: longword): boolean;

var
  filebuffer: array [0 .. 1024 * 16] of Byte;
  ...
  begin
    Result := crctable[Byte(CrcOld xor longword(b))] xor ((CrcOld shr 8) and $00FFFFFF)
  end;
...
begin
  ...
  try
    ...
    repeat
      ...
      rest   := rest - todo;
      aStream.ReadBuffer(filebuffer, todo);
...
class function TMuCRC32.GetCRC32(const aBytes: TBytes; var filecrc: longword): boolean;
var
  filebuffer: array [0 .. 1024 * 16] of Byte;
...
begin
  ...
  try
    ...
    repeat
      ...
      rest   := rest - todo;
      // aStream.ReadBuffer(filebuffer, todo);
      move(aBytes[c], filebuffer[0], todo);
      inc(c,todo);
      for i := 0 to (todo - 1) do
        Crc := RecountCRC(filebuffer[i], Crc);
    until (rest <= 0);
...
class function TMuCRC32.GetCRC32Pos(const aStream: tStream; startoffset, endoffset: int64;
...
begin
  ...
  try
    ...
    begin
      aStream.Seek(startoffset, TSeekOrigin.soBeginning);
...
class function TMuCRC32.GetCRC32Pos(const aFileName: string; startoffset, endoffset: int64;
  var filecrc: longword): boolean;
var
  stm: TBufferedFileStream;
begin
  stm := TBufferedFileStream.create(aFileName, fmOpenRead or fmShareDenyWrite);
  try
    Result := GetCRC32Pos(stm, startoffset, endoffset, filecrc);
  finally
    stm.Free;
  end;
end;

class function TMuCRC32.GetCRC32(const aFileName: string; var filecrc: longword): boolean;
var
  stm: TBufferedFileStream;
begin
  stm := TBufferedFileStream.create(aFileName, fmOpenRead or fmShareDenyWrite);
  try
    Result := GetCRC32(stm, filecrc);
  finally
    stm.Free;
  end;
