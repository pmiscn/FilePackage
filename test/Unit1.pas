...
begin
  c        := 0;
  FFStream := TfileStream.Create('e:\1638.mpkg', fmOpenRead or fmShareDenyRead);
...
var
  Package: TMFPackage;
begin
  Package               := TMFPackage.Create(getexepath + 'pb.mfpk', rwlRead);
...
begin
  ...
  Memo1.Lines.Add(format('findfile %d %d times,used %d secondes', [c, ct, GetTickCount - t]));
...
begin
  ...
  Memo1.Lines.Add(format('findfile %d %d times,used %d ms ', [c, ct, GetTickCount - t]));
...
var
  ...
  bs       : uint64;
begin
  IndexHash := TMFIndexHash.Create('e:\1638.mpkg', false, true,
...
var
  PackHeader: TMPackHeader;
  s         : ansistring;
  fn        : TfileStream;
var
  NamePos, NamePos2: TMFNamePos;
begin
  PackHeader.Clear;
  PackHeader.DespStr    := '123456789abc';
  PackHeader.VersionStr := '2.32.0';
  self.caption          := PackHeader.DespStr + '--' + PackHeader.VersionStr;
